...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        input = 5
        result = game.verify_input(min, max, input)
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_display_turn_order) { described_class.new(1, 10) }
    ...
    before do
      ...
      allow(game_display_turn_order).to receive(:display_guess)
    ...
    it 'increases guess_count by one' do
      expect { game_display_turn_order.display_turn_order(binary_search) }.to change { game_display_turn_order.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search).to receive(:make_guess).with(no_args).once
      game_display_turn_order.display_turn_order(binary_search)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(binary_search).to receive(:update_range).with(no_args).once
      game_display_turn_order.display_turn_order(binary_search)
    end
