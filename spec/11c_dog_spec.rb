...
describe Dog do
  ...
  subject(:old_yeller) { described_class.new('Old Yeller') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog shares the same method name as other classes' do
    include_examples 'shared method name'
  end

  it 'is not barking' do
    expect(old_yeller).not_to be_barking
  end

  it 'is sleeping' do
    expect(old_yeller).to be_sleeping
  end
end
