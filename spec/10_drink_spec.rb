...
describe Drink do
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_agua_fresca) { described_class.new('agua_fresca') }

      it 'is full' do
        expect(my_agua_fresca).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_horchata) { described_class.new('horchata', 10) }

      it 'is not full' do
        expect(my_horchata).not_to be_full
      end
    end
  end
end
